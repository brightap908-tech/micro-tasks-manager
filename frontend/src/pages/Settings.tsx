  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Database management, backups, and plugin configuration</p>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">
              ...
              Your encryption key is stored in <code className="text-brand-300 bg-slate-800 px-1 py-0.5 rounded text-xs">.encryption.key</code> — keep it safe and do not share it.
      ...
      <div className="card space-y-4">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <button className="btn-secondary flex items-center gap-2" onClick={() => refetchBackups()}>
              ...
              <tbody className="divide-y divide-slate-800">
                {backups.map(b => (
                  <tr key={b.filename} className="hover:bg-slate-800/30">
                    <td className="px-4 py-2.5 text-slate-300 font-mono text-xs">{b.filename}</td>
      ...
      <div className="card space-y-4">
        ...
        <div className="flex gap-3 items-center flex-wrap">
          <input
            type="file"
            accept=".db"
            className="text-sm text-slate-400 file:btn-secondary file:mr-3 file:cursor-pointer"
      ...
      <div className="card space-y-4">
        ...
        <button className="btn-secondary flex items-center gap-2" onClick={exportSettings}>
      ...
      <div className="card space-y-4">
        ...
        <p className="text-xs text-slate-500">
          ...
          <code className="bg-slate-800 px-1 rounded ml-1">backend/plugins/adapters/</code> and
          register them in the plugin registry.
        </p>
        <div className="space-y-2">
          {plugins.map(p => (
            <div key={p.plugin_id} className="flex items-center gap-3 bg-slate-800/50 rounded-lg px-4 py-3">
              <div className="w-2 h-2 rounded-full bg-green-500 shrink-0" />
              <div>
                <p className="text-sm font-medium text-slate-200">{p.plugin_name}</p>
                <p className="text-xs text-slate-500">{p.description}</p>
              </div>
              <code className="ml-auto text-xs text-slate-600 bg-slate-800 px-2 py-0.5 rounded">{p.plugin_id}</code>
            </div>
          ))}
        </div>
      </div>
