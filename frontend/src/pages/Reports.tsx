    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Earnings, productivity, and completion statistics</p>
        </div>
        <div className="flex gap-2">
          <button className="btn-secondary flex items-center gap-2" onClick={exportCSV}>
            <Download size={15} /> CSV
          </button>
          <button className="btn-secondary flex items-center gap-2" onClick={exportXLSX}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          ...
          {byWebsite.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={byWebsite} layout="vertical" margin={{ left: 20, right: 10 }}>
              ...
              <tbody className="divide-y divide-slate-800">
                {byWebsite.map(w => (
                  <tr key={w.website_id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="px-5 py-3 font-medium text-slate-200">{w.website_name}</td>
                    <td className="px-5 py-3 text-right text-slate-400">{w.task_count}</td>
                    <td className="px-5 py-3 text-right text-slate-400">{w.completed_count}</td>
                    <td className="px-5 py-3 text-right text-slate-400">
                      {w.task_count > 0 ? `${Math.round(w.completed_count / w.task_count * 100)}%` : '—'}
                    </td>
                    <td className="px-5 py-3 text-right text-green-400 font-semibold">${w.total_earnings.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
